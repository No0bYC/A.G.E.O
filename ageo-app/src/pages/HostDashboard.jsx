...
export default function HostDashboard() {
  ...
  useEffect(() => {
    ...
    supabase
      .from("access_codes")
      .select("id, label, check_in, check_out, status, created_at")
  ...
  async function generate(e) {
    ...
    setBusy(true);
    try {
      const { data, error: rpcError } = await supabase.rpc("generate_access_code", {
    ...
    <div className="flex flex-col" style={{ minHeight: "100vh" }}>
      <header className="px-5 pt-6 pb-4" style={{ background: C.ink }}>
        <div className="flex items-center justify-between max-w-3xl mx-auto w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="ageo-display text-lg leading-none" style={{ color: C.white }}>{host ? host.name : "Espace hôte"}</p>
              <p className="text-xs mt-1" style={{ color: C.white, opacity: 0.6 }}>{properties.length} propriété{properties.length !== 1 ? "s" : ""}</p>
            </div>
          </div>
      ...
      <main className="flex-1 px-5 py-5">
        <div className="max-w-3xl mx-auto">
          {properties.length === 0 ? (
            <p className="text-sm" style={{ color: C.ink, opacity: 0.6 }}>Aucune propriété pour le moment.</p>
          ) : (
            <>
              {properties.length > 1 && (
                <div className="flex gap-2 mb-4 overflow-x-auto ageo-scroll pb-1">
                  {properties.map((p) => (
                    <button
                      ...
                      style={{ borderColor: selectedId === p.id ? C.sky : C.line, background: selectedId === p.id ? C.sky : C.white, color: selectedId === p.id ? C.white : C.ink }}
                    >
                      {p.name}
                    </button>
                  ))}
              ...
              <form onSubmit={generate} className="rounded-2xl border p-4 mb-6" style={{ borderColor: C.line, background: C.white }}>
                ...
                <Field label="Note (optionnel)">
                  <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder="Ex. Famille Dupont" className={inputCls} style={inputStyle} />
                </Field>
              ...
              {lastCode && (
                <div className="rounded-2xl p-4 mb-6 text-center" style={{ background: C.skyWash }}>
                  <p className="text-xs font-bold mb-1" style={{ color: C.sky }}>Nouveau code à transmettre</p>
                  <p className="ageo-display text-3xl tracking-widest" style={{ color: C.sky }}>{lastCode}</p>
                </div>
              )}
                ...
                {codes.map((c) => (
                  <div key={c.id} className="rounded-2xl border p-3 flex items-center justify-between" style={{ borderColor: C.line, background: C.white }}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        {c.label && <p className="text-sm font-bold" style={{ color: C.ink }}>{c.label}</p>}
                        <p className="text-xs" style={{ color: C.ink, opacity: 0.5 }}>{c.check_in} → {c.check_out}</p>
                      </div>
                    </div>
                    <span className="text-xs font-bold px-2 py-1 rounded-full" style={{ background: c.status === "used" ? "#3FA37722" : C.canaryWash, color: c.status === "used" ? "#3FA377" : C.canaryDeep }}>
                      {c.status === "used" ? "Actif" : c.status === "revoked" ? "Révoqué" : "En attente"}
                    </span>
                  </div>
                ))}
              </div>
