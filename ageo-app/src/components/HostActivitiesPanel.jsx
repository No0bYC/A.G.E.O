...
import { ACTIVITY_TYPES, REGIONS, typeInfo, signedActivityPhotoUrl, uploadActivityPhoto } from "../lib/activities.js";
import { C, IconBadge, Field, PrimaryButton, SecondaryButton, inputCls, inputStyle } from "./ui.jsx";
import { Trash2, X, Check, Search, ArrowLeft, Waves, ImagePlus } from "lucide-react";
...
function mapsLink(name) {
  return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(name + " Mauritius")}`;
}

/* ---------------- Sélecteur : jusqu'à 10 activités du catalogue ---------------- */

function ActivityPickerModal({ propertyId, catalog, pickedIds, onToggle, onClose }) {
  ...
  async function handleToggle(activityId, isPicked) {
    setBusyId(activityId);
    await onToggle(activityId, isPicked);
    setBusyId(null);
  }
    ...
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: C.white }}>
      <div className="px-5 pt-6 pb-3" style={{ boxShadow: "0 1px 0 rgba(16,24,32,0.07)" }}>
        ...
        <div className="flex items-center justify-center mb-3">
          <span className="text-xs font-bold px-3 py-1 rounded-full" style={{ background: atMax ? C.canaryWash : C.skyWash, color: atMax ? C.canaryDeep : C.sky }}>
            {pickedIds.length} / 10 sélectionnées{atMax ? " — limite atteinte" : ""}
          </span>
        </div>
        <div className="relative mb-3">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher une activité..."
            className="w-full rounded-full border pl-9 pr-3 py-2 text-sm outline-none" style={{ borderColor: C.line, color: C.ink, background: C.neutral }} />
        </div>
      ...
      <div className="flex-1 overflow-y-auto ageo-scroll px-5 py-4">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center py-10 text-center">
            <IconBadge icon={Search} size={44} />
            <p className="text-sm mt-3" style={{ color: C.ink, opacity: 0.5 }}>Aucun résultat.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-3 max-w-3xl mx-auto">
            {filtered.map((a) => {
              ...
              const isPicked = pickedIds.includes(a.id);
              const disabled = (!isPicked && atMax) || busyId === a.id;
              return (
                <button key={a.id} type="button" onClick={() => !disabled && handleToggle(a.id, isPicked)} disabled={disabled}
                  className="text-left rounded-3xl overflow-hidden ageo-card relative" style={{ opacity: !isPicked && atMax ? 0.4 : 1 }}>
                  <div className="flex items-center justify-center relative" style={{ height: 90, background: C.skyWash }}>
                    <TIcon size={26} color={C.sky} />
                    <div className="absolute top-2 right-2 rounded-full flex items-center justify-center" style={{ width: 24, height: 24, background: isPicked ? C.sky : "rgba(255,255,255,0.85)" }}>
                      {isPicked && <Check size={14} color={C.white} />}
                    </div>
                  </div>
                  <div className="px-3 py-2.5">
                    <p className="text-sm font-bold leading-snug" style={{ color: C.ink }}>{a.name}</p>
                    <div className="flex items-center gap-1 mt-1.5">
                      <span className="text-xs font-bold px-1.5 py-0.5 rounded-full" style={{ background: C.canaryWash, color: C.canaryDeep }}>{a.region}</span>
      ...
      <div className="px-5 py-4" style={{ boxShadow: "0 -1px 0 rgba(16,24,32,0.06)" }}>
        <div className="max-w-3xl mx-auto"><PrimaryButton onClick={onClose} full>Valider ma sélection</PrimaryButton></div>
      </div>
    </div>
  );
}

/* ---------------- Formulaire de création (illimité) ---------------- */
...
function ActivityFormModal({ propertyId, onClose, onSaved }) {
  ...
  function handleNameChange(v) {
    setName(v);
    if (!mapsTouched) setMapsUrl(v ? mapsLink(v) : "");
  }
  ...
  async function submit(e) {
    ...
    try {
      let photoPath = null;
      if (file) photoPath = await uploadActivityPhoto(propertyId, file);
      const { error: insertError } = await supabase.from("property_custom_activities").insert({
        property_id: propertyId,
        name: name.trim(),
        description: description.trim(),
        type, region,
        maps_url: mapsUrl || mapsLink(name),
        ref_url: refUrl.trim(),
      ...
      <form onSubmit={submit} className="rounded-3xl w-full overflow-y-auto ageo-scroll" style={{ background: C.white, maxWidth: 480, maxHeight: "88vh" }} onClick={(e) => e.stopPropagation()}>
        <div className="p-5">
          <div className="flex items-center justify-between mb-4">
            <span className="ageo-display text-lg" style={{ color: C.ink }}>Nouvelle activité</span>
            <button onClick={onClose} aria-label="Fermer" type="button"><X size={18} color={C.ink} style={{ opacity: 0.5 }} /></button>
          </div>
          ...
          <Field label="Nom de l'activité">
            <input value={name} onChange={(e) => handleNameChange(e.target.value)} placeholder="Ex. Île aux Cerfs" className={inputCls} style={inputStyle} required />
          </Field>
          <Field label="Description">
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="Une ou deux phrases pour donner envie..." className={inputCls} style={{ ...inputStyle, resize: "none" }} />
          </Field>

          <p className="text-xs font-bold uppercase tracking-wide mb-2" style={{ color: C.sky }}>Type d'activité</p>
          <div className="flex flex-wrap gap-2 mb-4">
            {ACTIVITY_TYPES.map((t) => (<Chip key={t.key} active={type === t.key} onClick={() => setType(t.key)} icon={t.icon}>{t.label}</Chip>))}
          </div>
          ...
          <Field label="Lien Google Maps">
            <input value={mapsUrl} onChange={(e) => { setMapsUrl(e.target.value); setMapsTouched(true); }} className={inputCls} style={inputStyle} />
          </Field>
          <Field label="Site de référence">
            <input value={refUrl} onChange={(e) => setRefUrl(e.target.value)} placeholder="https://..." className={inputCls} style={inputStyle} />
          </Field>
...
/* ---------------- Ligne de la sélection courante ---------------- */

function HostActivityRow({ activity, source, onRemove }) {
  const info = typeInfo(activity.type);
  const TIcon = info.icon;
  const [photoUrl, setPhotoUrl] = useState(null);
  useEffect(() => { if (activity.photo_path) signedActivityPhotoUrl(activity.photo_path).then(setPhotoUrl); }, [activity.photo_path]);

  return (
    <div className="rounded-2xl border p-3 flex items-center gap-3" style={{ borderColor: C.line, background: C.white }}>
      <div className="rounded-xl overflow-hidden shrink-0 flex items-center justify-center" style={{ width: 52, height: 52, background: C.skyWash }}>
        {photoUrl ? <img src={photoUrl} alt="" className="w-full h-full object-cover" /> : <TIcon size={20} color={C.sky} />}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mt-1">
          ...
          <span className="text-xs" style={{ color: C.ink, opacity: 0.45 }}>{info.label}</span>
          <span className="text-xs font-bold" style={{ color: source === "custom" ? C.sage : C.sky, opacity: 0.8 }}>
            {source === "custom" ? "· Créée par vous" : "· Depuis la liste"}
          </span>
        </div>
      </div>
...
export default function HostActivitiesPanel({ propertyId }) {
  ...
  async function togglePick(activityId, isPicked) {
    if (isPicked) {
      await supabase.rpc("remove_activity_pick", { p_property_id: propertyId, p_activity_id: activityId });
      setPickedIds((prev) => prev.filter((id) => id !== activityId));
    } else {
      const { data } = await supabase.rpc("add_activity_pick", { p_property_id: propertyId, p_activity_id: activityId });
      if (data && data.ok) setPickedIds((prev) => [...prev, activityId]);
    }
  }
  async function removeCustom(id) {
    await supabase.from("property_custom_activities").delete().eq("id", id);
    ...
    <div>
      <div className="flex items-center justify-between mb-1 flex-wrap gap-2">
        ...
        <div className="flex gap-2">
          <SecondaryButton onClick={() => setShowPicker(true)}>Choisir dans la liste</SecondaryButton>
          <PrimaryButton onClick={() => setShowForm(true)}>+ Créer une activité</PrimaryButton>
        </div>
      </div>
      <p className="text-xs mb-5" style={{ color: C.ink, opacity: 0.6 }}>
        {pickedIds.length}/10 issues de la liste · {customActivities.length} créée{customActivities.length !== 1 ? "s" : ""} par vous · {combined.length} au total pour vos voyageurs
      </p>

      {combined.length === 0 ? (
        <div className="flex flex-col items-center py-10 text-center rounded-3xl" style={{ background: C.skyWash }}>
          <IconBadge icon={Waves} size={48} />
          <p className="text-sm mt-3 font-bold" style={{ color: C.ink }}>Aucun bon plan pour l'instant</p>
          <p className="text-xs mt-1" style={{ color: C.ink, opacity: 0.6, maxWidth: 240 }}>Piochez jusqu'à 10 activités dans la liste, ou créez les vôtres.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {combined.map(({ activity, source }) => (
            <HostActivityRow key={activity.id} activity={activity} source={source} onRemove={() => (source === "custom" ? removeCustom(activity.id) : togglePick(activity.id, true))} />
          ))}
        </div>
      )}

      {showForm && <ActivityFormModal propertyId={propertyId} onClose={() => setShowForm(false)} onSaved={() => { setShowForm(false); reload(); }} />}
      {showPicker && <ActivityPickerModal propertyId={propertyId} catalog={catalog} pickedIds={pickedIds} onToggle={togglePick} onClose={() => setShowPicker(false)} />}
    </div>
  );
}
