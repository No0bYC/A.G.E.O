...
import { ACTIVITY_TYPES, REGIONS, typeInfo, signedActivityPhotoUrl } from "../lib/activities.js";
import { C, IconBadge, EmptyState } from "./ui.jsx";
import { MapPin, ExternalLink, X, Waves } from "lucide-react";
...
function ActivityCard({ activity, onOpen }) {
  ...
  return (
    <button onClick={onOpen} className="text-left rounded-3xl overflow-hidden ageo-card">
      <div className="flex items-center justify-center" style={{ height: 90, background: C.skyWash }}>
        {activity.photoUrl ? <img src={activity.photoUrl} alt={activity.name} className="w-full h-full object-cover" /> : <TIcon size={26} color={C.sky} />}
      </div>
...
function ActivityDetailSheet({ activity, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end" style={{ background: "rgba(28,31,38,0.4)" }} onClick={onClose}>
      <div className="ageo-sheet rounded-t-3xl" style={{ background: C.white, maxHeight: "82vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5">
          ...
          <div className="rounded-2xl overflow-hidden mb-4" style={{ aspectRatio: "16/9", background: C.skyWash }}>
            {activity.photoUrl ? <img src={activity.photoUrl} alt="" className="w-full h-full object-cover" /> : (
              <div className="w-full h-full flex items-center justify-center"><TIcon size={40} color={C.sky} /></div>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1" style={{ background: C.skyWash, color: C.sky }}>
              <TIcon size={12} /> {info.label}
            </span>
          </div>
          {activity.description && <p className="text-sm mb-5" style={{ color: C.ink, opacity: 0.75, lineHeight: 1.5 }}>{activity.description}</p>}
          <div className="space-y-2">
            {activity.maps_url && (
              <a href={activity.maps_url} target="_blank" rel="noreferrer" className="w-full flex items-center justify-center gap-2 rounded-full font-bold text-sm py-2.5" style={{ background: C.canary, color: C.ink }}>
                <MapPin size={15} /> Voir sur Google Maps
              </a>
            )}
            {activity.ref_url && (
              <a href={activity.ref_url} target="_blank" rel="noreferrer" className="w-full flex items-center justify-center gap-2 rounded-full font-bold text-sm py-2.5 border" style={{ borderColor: C.sky, color: C.sky }}>
                <ExternalLink size={14} /> Site de référence
              </a>
            )}
          </div>
        </div>
...
export default function ActivityBrowser({ propertyId }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const withPhotos = await Promise.all(
        merged.map(async (a) => ({ ...a, photoUrl: a.photo_path ? await signedActivityPhotoUrl(a.photo_path) : null }))
      );
  ...
  if (loading) {
    return <p className="text-sm text-center py-10" style={{ color: C.ink, opacity: 0.5 }}>Chargement...</p>;
  }

  if (activities.length === 0) {
    return <EmptyState icon={MapPin} title="Aucun bon plan pour l'instant" subtitle="Votre hôte n'a pas encore sélectionné d'activités." />;
  }
    ...
    <>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center py-10 text-center">
          ...
          <p className="text-sm mt-3" style={{ color: C.ink, opacity: 0.5 }}>Aucune activité pour cette combinaison de filtres.</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-3">
          {filtered.map((a) => (<ActivityCard key={a.id} activity={a} onOpen={() => setOpenId(a.id)} />))}
        </div>
      )}
